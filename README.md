...
### Map Markers
- Each location is represented by a colored circle marker
- Color indicates forecast rating
- Marker size is consistent for easy visibility
- Size: 15px radius with 2px border

### Popup Information
When clicking a marker, a detailed popup appears with:
- Location name (large heading)
- Rating with color indicator
- Full summary text
- Detailed tide information table
- Wind details
- Water temperature
- Current information
- Additional notes and insights
...
Edit the `create_map()` function in `app.py` to modify:
- Zoom level (currently 9)
- Tile provider (currently OpenStreetMap)
- Marker colors and sizes
- Legend position and styling

### Adding More Data
Simply update `kayak_forecast.json` with additional locations following the same format.
